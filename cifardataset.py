...
class CIFAR10SUB(VisionDataset):
    ...
    def __init__(self, root, classselect, train=True, transform=None, target_transform=None,
                 download=False, sample=False, samplenum=5):

        super(CIFAR10SUB, self).__init__(root, transform=transform,
        ...
        for index in range(len(classselect)):
            targetindex = [i for i,x in enumerate(targets) if x==self.classselect[index]]
            if self.sample:
                targetindex = targetindex[0:samplenum]
                targetselect = [index for i in range(samplenum)]
            else:
                targetselect = [index for i,x in enumerate(targets) if x==self.classselect[index]]
        ...
        self._load_meta(self.classselect)
