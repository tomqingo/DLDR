...
f = open('./save_convnet/param_data_100.txt','rb')  
...
variance_ratio_top = variance_ratio[0:4]
#pdb.set_trace()
variance_ratio_rest = np.sum(variance_ratio[4:])

variance_ratio_top = np.hstack([variance_ratio_top, variance_ratio_rest])

label_list = ['1st', '2nd', '3rd', '4th', 'others']

x_axis = list(range(len(label_list)))
print(variance_ratio_top)
rects1 = plt.bar(x=x_axis, height=variance_ratio_top, width=0.4, color='blue', label='PCA ratio distribution')
plt.ylabel('Percentage(%)')
plt.xticks(x_axis, label_list)
plt.legend()
for rect in rects1:
    height = rect.get_height()
    plt.text(rect.get_x() + rect.get_width() / 2, height, str(height)[0:5], ha="center", va="bottom")
plt.savefig('pca_ratio.png')
...
def main():
    ...
    if 'convnet' in args.arch:
        model = convNet.convnet(num_classes=args.num_class)
    ...
    # optimizer warm_up+cosine_decay
    # warm_up = int(0.1*args.epochs)
    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    #lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(args.epochs*1.05), last_epoch=args.start_epoch-1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #        param_group['lr'] = ((epoch+1)/warm_up)*args.lr
        #else:
        #    lr_scheduler.step()
    ...
    io.savemat('./'+args.save_dir+'/testaccsub.mat', mdict={'testaccsub':test_acc_col})
