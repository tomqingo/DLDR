...
parser.add_argument('-k', '--k-shot', default=1, type=int,
...
def main():
    ...
    elif 'sim' in args.arch:
        model = convNetsim.convnetsim(args.num_class)
    model.cuda()
    param_num = sum(x.numel() for x in model.parameters() if x.requires_grad)
    print(param_num)
    #pdb.set_trace()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if (epoch >= 50):
        #     param_vec.append(get_model_param_vec(model))
        train_acc_col.append(train_prec1)
        test_acc_col.append(prec1)
    
    io.savemat('./'+args.save_dir+'/'+args.arch+'trainacc.mat', mdict={args.arch+'trainacc':train_acc_col})
    io.savemat('./'+args.save_dir+'/'+args.arch+'testacc.mat', mdict={args.arch+'testacc':test_acc_col})
